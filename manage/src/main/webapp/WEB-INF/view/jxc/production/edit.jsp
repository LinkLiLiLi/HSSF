...
<div class="panel admin-panel">
  <div class="panel-head"><strong><span class="icon-pencil-square-o"></span>&nbsp;&nbsp;编辑生产单</strong></div>
  <div class="body-content">
    <form action="jxc/production/edit" method="post" class="form-x" id="form-article-add">
      <input name="id" value="${production.id}" type="hidden"/>
        ...
        <div class="field">
          <select name="productType.id" class="input" id="type" onchange="changemodel('type')">
			...
			<c:forEach items="${typeList }" var="type">
				<option value="${type.id}" <c:if test="${production.productType.id == type.id }">selected</c:if>>${type.typeName }</option>
			</c:forEach>
		  </select>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="field">
          <select name="productStore.id" class="input" id="typemodel">
			...
			<c:forEach items="${production.valueList }" var="value">
				<option value="${value.pid}" <c:if test="${production.productStore.id == value.pid }">selected</c:if>>${value.pname }</option>
			</c:forEach>
		  </select>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input" id="purpose" name="purpose" value="${production.purpose }"/>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input" id="amount" name="amount" value="${production.amount}" onblur="changeamount()"/>
        </div>
      </div>
      ...
      <div class="form-group">
        ...
        <div class="field">
          <input type="text" class="input" id="remark" name="remark" value="${production.remark}"/>
        </div>
      </div>
	  ...
	  <div class="form-group">
        ...
        <div class="field">
          <table class="table table-hover text-center">
		     ...
		     <tbody id="items">
		     	<c:forEach items="${production.itemList}" var="item" varStatus="status">
		     		<tr>
		     			<td>
		     				<input type='hidden' name='itemList[${status.index}].productType.id' class='input' value='${item.productVo.productId}' />
		     				${item.productVo.productName}
		     			</td>
		     			<td>
		     				<select name='itemList[${status.index}].productStore.id' class='input' id='storeuseamount${status.index}' onchange='changestore("storeuseamount${status.index})'>
		     					<option value=''>请选择型号</option>
		     					<c:forEach items="${item.productVo.valueList}" var="valls">
		     						<option value='${valls.pid }' <c:if test="${item.productStore.id == valls.pid }">selected</c:if>>${valls.pname}</option>
		     					</c:forEach>
		     				</select>
		     			</td>
		     			<td>
		     				<input type='text' name='itemList[${status.index}].amount' class='input' value='${item.amount }' id='useamount${status.index}' onblur='changeuseamount("useamount${status.index}")'/>
		     				<input type='hidden' id='storeamount${status.index}' class='input' value='${item.productVo.storeAmout}'/>
		     				<input type='hidden' id='amount${status.index}' class='input' value='${item.productVo.useAmount}'/>
		     			</td>
		     		</tr>
		     	</c:forEach>
			</tbody>
